...
int16_t FTemperature = 25;
uint16_t FCurrentSpeed = 120;
uint32_t FDelay = 100;
...
static void itoa(s16 n, char s[]) {
	...
	do {       /* генерируем цифры в обратном порядке */
		s[i++] = (uint8_t)(n % 10 + '0');   /* берем следующую цифру */
...
bool Gui::getStringSpeed(cph::TTextBlock<DisplayType>* self, char* value) {
	static uint16_t FtempSpeed = 0;
	uint16_t lCurrentSpeed = FCurrentSpeed;

	if (!self->is_inited() || FtempSpeed != lCurrentSpeed) {
		FtempSpeed = lCurrentSpeed;
		itoa((s16)FtempSpeed, value);
		strcat(value, " km/h");
		return true;
	}
