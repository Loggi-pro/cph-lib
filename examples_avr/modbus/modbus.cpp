...
typedef cph::UsartAsync<cph::Usart0, 255> Usart;
...
struct TransportAdapterImpl : TransportAdapter {
	...
	virtual bool init() override {
		Usart::init<BAUDRATE>();
		Usart::setOnReceive(&onReceive);
		return true;
	}
	virtual  bool writeByte(u08 byte) override {
		return Usart::writeByte(byte);
	}
...
int main() {
	...
	Usart::setOnReceive(&transport.onReceive);
	cph::SystemCounter::initPrecision<cph::TimerPrecision::Resolution_1ms, cph::TTimerNumber::TIMER_0>();
	cph::SystemCounter::enable < cph::TTimerNumber::TIMER_0>();
