		...
		namespace Private {
			...
			struct CheckSameConfigImp<Typelist<Head, Tail>, CurrentConfig> {
				static const bool value =
				    CheckSameConfigHelper<Typelist<Head, Tail>, CurrentConfig, typename Head::Configuration>::value;
			};
			...
			class Shifter {
					...
					typedef vd::static_if_v<ShiftDirection, LeftShifter1, RightShifter2> FirstShifter;
					typedef vd::static_if_v<ShiftDirection, RightShifter1, LeftShifter2> SecondShifter;

					typedef vd::static_if_v<mapDirection, FirstShifter, SecondShifter> ActualShifter;
				public:
					template<class T>
					static T Shift(T value) {
			...
			struct TakeFirst<Typelist<Head, Tail>, Num> {
				typedef Typelist < Head,
				        typename TakeFirst < Tail, (uint8_t)(Num - 1) >::Result
				        > Result;
			};
			////////////////////////////////////////////////////////////////////////////////
			...
			struct SkipFirst<Typelist<Head, Tail>, Num> {
				typedef typename SkipFirst < Tail, uint8_t(Num - 1) >::Result Result;
			};
			...
			struct PortWriteIterator< Typelist<Head, Tail>, PinList, ValueType> {
				...
				typedef vd::static_if_v<sizeof(PortType) >= sizeof(ValueType),
				        PortType, ValueType > DataType;
			...
			struct PortConfigurationIterator< Typelist<Head, Tail>, PinList, Configuration, config, ValueType> {
				...
				static void SetConfiguration() {
					...
					PortConfigurationIterator<Tail, PinList, Configuration, config, ValueType>::template
					SetConfiguration<mask>();
				}
			};
		}
	}
